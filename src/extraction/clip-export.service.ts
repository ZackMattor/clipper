...
export class ClipExportService {
  ...
  async run(options: ClipExportOptions): Promise<ClipRunMetadata> {
    ...
    const metadata = await extractClipsInternal({
      ...
      ffmpegStats: options.ffmpegStats ?? true,
...
function buildVideoCodecArgs(): string[] {
...
function normalizeOptions(options: NormalizedInput): RequiredClipOptions {
  const subtitleFiles = options.subtitleFiles?.map((file) => path.resolve(file));
  const subtitleVideoMap = new Map(
  ...
  return {
    mediaRoot: path.resolve(options.mediaRoot ?? process.env.MEDIA_ROOT ?? 'src_media'),
...
async function extractClipsInternal(options: NormalizedInput): Promise<ClipRunMetadata> {
